...
hs = now.hour()+ ( float ) now.minute()/60+  ( float )now.second()/6000;   // hora input para el calculo de la temperatura instantanea
                        ...
                        if  ( test==true) {//boolean test=true; PARA DESARROLLO MONITOREADO
                                              ...
                                              Serial.print (F("hora Calc=  "));Serial.print(now.hour());Serial.print (":");Serial.print(now.minute());Serial.print (":");Serial.print(now.second());
...
//T_aNoche=           12   * sin(  (PI*(11.9              ))     / (12    + 2*1.5) )+  4;
  T_aNoche=(deltaT) * sin(  (PI*(aNochecer-aManecer-0.1)) / (deltaT+(2*a)) ) + tempMin; //calc temp  Tdia la ultima hora del dia=aNochecer
                      ...
                      if ( test==true){   Serial.print   (F("deltaT= "));Serial.print(deltaT,4);Serial.print(F("  tempMax="));;Serial.println(tempMax);Serial.print("  tempMin=");;Serial.println(tempMin);
                                          Serial.print   (F("      T_aNoche= "));Serial.print(T_aNoche,4);Serial.println(F("  ultima hora del dia= es el aNochecer"));
                                          Serial.println (F("-------------------------------------------------"));
